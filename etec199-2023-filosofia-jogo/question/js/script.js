class Pergunta {
  ...
  resp_option(resposta) {
    if (resposta === this.a) {
      return "a";
    } else if (resposta === this.b) {
      return "b";
    } else if (resposta === this.c) {
      return "c";
    } else if (resposta === this.d) {
      return "d";
    }
  }
}
...
function showQuestion(index) {
  ...
  createButton(question.a, optionsElement, "a");
...
function checkAnswer(answer) {
  ...
  const option = question.resp_option(answer)
  const optionCorreta = question.correta

  if (question.resp_verifica(answer)) {
    document.getElementById(option).style.backgroundColor = "#00b19d";
    document.getElementById(`progressBar${currentQuestionIndex}`).style.backgroundColor = "#4caf50"
  } else {
    document.getElementById(option).style.backgroundColor = "red";
    document.getElementById(optionCorreta).style.backgroundColor = "#00b19d";
  ...
  // Adicionar um timeout de 3 segundos para mostrar a div com display: none
  setTimeout(function() {
